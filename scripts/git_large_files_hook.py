...
def git_filesize_hook(megabytes_cutoff=5, verbose=False):
    ...
    if too_big:
        msg = ("ERROR: your commit contains %s file%s that exceed%s size "
               "limit of %d megabyte%s:\n%s")
        msg = msg % (len(too_big), "" if len(too_big) < 2 else "s", "s" if len(too_big) < 2 else "s", megabytes_cutoff, "" if megabytes_cutoff < 2 else "s", "\n".join(sorted(too_big)))
        msg += "\n\nUse the '--no-verify' option to by-pass this hook"
        return msg
    return 0

if __name__ == "__main__":
    sys.exit(git_filesize_hook(1))
