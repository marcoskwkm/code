...
vector <int> AllGraphCuts::findGraph(vector <int> x) {
    ...
    for (int u = 0; u < n; u++) {
        cmp[u] = u;
        for(int v = u + 1; v < n; v++) {
            if (x[u*n + v] != x[v*n + u]) return vector<int> {-1};
            edges.push_back(Edge(x[u*n + v], u, v));
            for (int w = v + 1; w < n; w++) {
                if (x[u*n + w] != min(x[u*n + v], x[v*n + w])) {
                    if (x[u*n + v] != x[v*n + w]) {
                        // printf("%d-%d: %d\n%d-%d: %d\n%d-%d: %d\n",
                        //        u, w, x[u*n+w],
                        //        u, v, x[u*n+v],
    ...
    for (Edge edge: edges) {
        ...
        ret.push_back(x[u*n + v]*n*n + u*n + v);
        join(u, v);
    }
    //assert(ret.size() == n - 1);
    return ret;
